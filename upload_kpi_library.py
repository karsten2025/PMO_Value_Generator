#!/usr/bin/env python3
"""
PMO KPI Library Upload Script
Lädt 30 KPIs (3 pro Impact Cycle Schritt) in die Supabase-Datenbank hoch.

Jede KPI folgt der 2x3 Matrix-Regel:
- Sprachen: DE, EN, ES
- Register: Colloquial, Management

KPI-Typen:
- Strategic: Fokus auf Alignment & Value
- Tactical: Fokus auf Kapazität & Governance  
- Operational: Fokus auf Effizienz & Qualität
"""
...
if not SUPABASE_URL or not SUPABASE_KEY:
    raise ValueError("SUPABASE_URL und SUPABASE_KEY müssen in .env definiert sein!")

supabase: Client = create_client(SUPABASE_URL, SUPABASE_KEY)


# ============================================================================
# KPI-BIBLIOTHEK: 30 Kennzahlen für den PMO Impact Cycle
# ============================================================================
...
def upload_kpis():
    """
    Lädt alle 30 KPIs in die Supabase pmo_kpi_library-Tabelle hoch.
    """
    print("=" * 80)
    print("PMO KPI LIBRARY UPLOAD")
    print("=" * 80)
    print(f"\n📊 Anzahl KPIs zu uploaden: {len(KPI_LIBRARY)}")
    print(f"🔗 Ziel-Tabelle: pmo_kpi_library")
    print(f"🌐 Supabase URL: {SUPABASE_URL}\n")

    try:
        # Upload durchführen
        response = supabase.table("pmo_kpi_library").insert(KPI_LIBRARY).execute()
        
        print("✅ Upload erfolgreich!")
        print(f"📦 {len(response.data)} KPIs wurden hochgeladen.")
        
        # Zusammenfassung nach Typ
        strategic = len([k for k in KPI_LIBRARY if k["kpi_type"] == "strategic"])
        tactical = len([k for k in KPI_LIBRARY if k["kpi_type"] == "tactical"])
        operational = len([k for k in KPI_LIBRARY if k["kpi_type"] == "operational"])
        
        print("\n📈 Zusammenfassung nach KPI-Typ:")
        print(f"   - Strategisch:  {strategic}")
        print(f"   - Taktisch:     {tactical}")
        print(f"   - Operativ:     {operational}")
        print(f"   - GESAMT:       {strategic + tactical + operational}")
...
def print_kpi_overview():
    """
    Gibt eine formatierte Übersicht aller KPIs auf der Konsole aus.
    Hilfreich für Dokumentation und Review.
    """
    print("\n" + "=" * 80)
    print("KPI-BIBLIOTHEK ÜBERSICHT")
    print("=" * 80 + "\n")
    ...
    for kpi in KPI_LIBRARY:
        ...
        if kpi["step_number"] != current_step:
            ...
            print(f"\n{'─' * 80}")
            print(f"STEP {current_step}: {kpi['internal_code']}")
            print(f"{'─' * 80}\n")
        ...
        print(f"{kpi_type_label[kpi['kpi_type']]} | {kpi['title']}")
        print(f"   Code: {kpi['kpi_code']}")
        print(f"   Unit: {kpi['unit']}")
        print(f"   DE (Normalo):    {kpi['matrix_data']['de']['colloquial'][:80]}...")
        print(f"   DE (Management): {kpi['matrix_data']['de']['management'][:80]}...")
        print()
...
if __name__ == "__main__":
    print("\n🚀 PMO KPI Library Upload Script gestartet...\n")
    
    # Optionale Übersicht anzeigen
    print_kpi_overview()
    
    # Upload durchführen
    confirm = input("\n⚠️  Möchten Sie die KPIs jetzt in Supabase hochladen? (ja/nein): ")
